...
var examClient = {
	...
	examClient.init = function(options) {
		...
		this.flvPlayer ;
		//UE配置文件  
		this.UEConfig = {toolbars: [[ 'simpleupload','kityformula','scrawl','photo','fullscreen','qr-code']],autoHeightEnabled: false,autoFloatEnabled: true,serverUrl: top.baseURL ,enableContextMenu: false,initialStyle:'p{line-height:1em} img{max-width:88%}',enableAutoSave: false,autoHeightEnabled:false};
				...
				error : function(event){
					...
					if(undefined != event.jPlayer && undefined != event.jPlayer.error && undefined != event.jPlayer.error.type){
						if(event.jPlayer.error.type == "e_no_solution" || event.jPlayer.error.type == "e_flash" || event.jPlayer.error.type == "e_flash_disabled" || event.jPlayer.error.type == "e_version"){
							top.topAlert("提示","页面上插入Flash时出现问题，是否安装了FLASH，是否禁用了FLASH？","确定",function(){
							...
							top.topAlert("提示","由于未设置媒体，因此媒体无法播放。","确定",function(){});
						}else if(event.jPlayer.error.type == "e_no_support"){
							top.topAlert("提示","无法播放的媒体格式，"+examClient.videoPlayArray[examClient.playNow]+"。","确定",function(){});
						}else{
							top.topAlert("提示","由于网络原因音频播放中断，点击确定重试。","确定",function(){
								try {
...
//		try {
			...
			$.post( baseURL + "/examPaper/checkVideoPlayStaus.json", {
				...
				if(!(data.playStatus)  || "1"==paperView){
					data.playStatus="-1";	
				}
		        //播放中状态
...
//		console.log(aitems.toString());
		...
		for(var kk = 0 ; kk<count ;kk++){
			...
			//原有代码是直接替换右括号，F) 渴望(成就)；有志(成为) 时出现了bug，又不敢直接删除原代码，所以盲猜当初是为了删除第一个右括号
			if(aitemValue.indexOf(")")==0){
...
//			如果question.answerOption不是数组那么就要转换成数组
	...
	//			if (("0".equals(orderChangeStatus) && "1".equals(randomOption)) || "1".equals(orderChangeStatus) || "3".equals(orderChangeStatus)) {
				//[{"value":"0","key":"由考试规则确定"},{"value":"1","key":"子题和选项都异序"},{"value":"2","key":"仅子题异序"},{"value":"3","key":"仅选项异序"},{"value":"4","key":"子题和选项都不异序"} ]
				if("1"==this.randomOption&&("0"==this.orderChangeStatus||"1"==this.orderChangeStatus||"3"==this.orderChangeStatus)){
...
//			if(this.isSplitShowQuestion||(parentQuestion.questionType.isComplex&&parentQuestion.questionType.isComplex=="1")){
		...
		else if(answerType=="2"||answerType=="10"){
	...
	examClient.buildExamPaperContent2HTML = function(subExamPaperContentList) {
		...
		for (var i=0;i<subExamPaperContentList.length;i++) {
			...
			//复合题型
				...
				//[{"value":"0","key":"由考试规则确定"},{"value":"1","key":"子题和选项都异序"},{"value":"2","key":"仅子题异序"},{"value":"3","key":"仅选项异序"},{"value":"4","key":"子题和选项都不异序"} ]
				if("1"==this.randomQuestion && ("0"==this.orderChangeStatus || "1"==this.orderChangeStatus||"2"==this.orderChangeStatus) ){
...
//				var subQuestionArray = question.subQuestionArray;
				...
				for (var subQuestion in question.subQuestionArray) {
					this.buildQuestion2html(question , question.subQuestionArray[subQuestion],answerAreaHtml);
				}
...
//		top.sysAlert(this.videoPlayArray.toString());
	...
	examClient.showQuestionContent = function() {
		...
		if((!(this.isSplitShowQuestion&&(this.curPartAnswerQuestionPos<this.curPartSubQuestionNum)))&&(this.maxPartNum==(this.partNum+1))){
			...
			examClient.switchSubmitPaperBtnStatus(top.examTime);
...
//				obj.className = "input_changed";
	...
	examClient.submitTestpaper = function() {
		...
		// 限制提前交卷
		if($('#id_a_submitPaper').hasClass('disabled-click')){
			parent.topAlert2Close("提示", "考试结束前" + top.submitPaperLimit + "分钟内允许考生交卷，请耐心等待", function() {})
			return false;
		}
		...
		parent.topAlert2Close("提示",message,"确定",function() {
			$.post("saveStudentAnswer.json?loginName="+studentId,
	    		  {studentResultId :studentExamResultId, examPaperId : exampid, batchId : batchId, studentId : studentId ,isTrain:top.window.isTrain},
	     		  function rendResponds(data) {
	    			var data1=data;
					if (data1.done == "failed") {
...
//		alert(this.isSplitShowQuestion&&(this.curPartSubQuestionNum>=this.curPartAnswerQuestionPos)&&(this.curPartAnswerQuestionPos!=1));
	...
	var studentId = parent.studentId , realName=(parent.real_name.length>1)?parent.real_name:"A";
	...
	examClient.subAnswer = function(qid, name, type) {
		...
		if( (values == "" || values.replace(/@!@/g, "") == "")  && "mcq"!=type) {	//"mcq"!=type 判断是多选题允许保存""
			return false;
		}
		//过滤特殊字符
		...
		$("#saveAnswer_loading").show();
		$.ajax({
			url : "preSaveStudentAnswer.json?loginName="+studentId+"&qid="+qid,// 要访问的后台地址
...
//			examClient.clickObj.className = "input_changed";
	...
	examClient.intiAnswer2_6 = function(questionIds) {
		...
		$("#openbg2").show();
	    try {
	        $.post("getStudentAnswer.json?loginName="+studentId, {
...
//				alert(obj.className);
	...
	examClient.switchSubmitPaperBtnStatus = function(examTime){
		// 设置了提前交卷限制的交卷按钮控制
		if(top.isSubmitPaperLimit === '1') {
			if(examTime / 60 > top.submitPaperLimit){
				$('#id_a_submitPaper').addClass('disabled-click').css('color','grey');
			} else {
				$('#id_a_submitPaper').removeClass('disabled-click').css('color','#444');
			}
		}
	}
	...
	function winSize_isSplit(){
	    ...
	    var rightWidth=$("#id_div_parttitle").width();
		  ...
		  if($(".exam_right-suo",window.parent.document).hasClass("active")){
			  $('#exam_right-container').find(".exam_right-wrap").addClass("active");
		      $(".exam_right-writ-answer").css({"left":"0","width":"968px","border-left":"1px solid #bae3ff","border-right":"1px solid #bae3ff"});
